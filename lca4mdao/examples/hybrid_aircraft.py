...
def build_data():
    ...
    aircraft.register()
    aircraft.delete(warn=False)
    aircraft.new_activity('hybrid_aircraft', name='hybrid_aircraft').save()
    aircraft.new_activity('hybrid_flight', name='hybrid_flight').save()
...
class Environment(ExplicitComponentLCA):
    ...
    def setup(self):
        ...
        # Outputs
        self.add_output('engine', units='kg', lca_parent=("aircraft", "hybrid_aircraft"), lca_units='kilogram',
                        lca_key=engine, val=0.)
        self.add_output('motor', units='kg', lca_parent=("aircraft", "hybrid_aircraft"), lca_units='kilogram',
                        lca_key=motor, val=0.)
        self.add_output('battery', units='kg', lca_parent=("aircraft", "hybrid_aircraft"), lca_units='kilogram',
        ...
        self.add_output('CO2', units='kg', lca_parent=("aircraft", "hybrid_flight"), lca_units='kilogram',
                        lca_key=CO2, exchange_type='biosphere', val=0.)
    ...
    def compute(self, inputs, outputs):
        ...
        amount_electricity = 0. * cycles
        amount_kerosene = inputs['kerosene'] * cycles
        amount_CO2 = amount_kerosene * 3.15

        outputs['engine'] = engine_weight
        # outputs['motor'] = motor_weight
        # outputs['battery'] = battery_weight
        # outputs['aluminium'] = structure_weight
        # outputs['electricity'] = amount_electricity
        # outputs['kerosene'] = amount_kerosene
        # outputs['CO2'] = amount_CO2
...
class ElectricTwinAnalysisGroup(Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('lca_data', Environment(cycles=self.options["cycles"]), promotes_outputs=["*"])
        self.add_subsystem('lca', GWP(), promotes=["*"])
        # self.connect("", "lca.engine")
        # self.connect("", "lca.motor")
        self.connect("ac|weights|W_battery", "lca_data.battery_weight")
        self.connect("cruise.OEW", "lca_data.structure_weight")
        # self.connect("", "lca.electricity")
        self.connect("descent.fuel_used_final", "lca_data.kerosene")
